...
function isoscelesHeight(halfWidth, degree) {
  return halfWidth * Math.atan(0.5 * radians(degree))
}
...
function pieSlice(segments, height, radius, tolerance) {
  ...
  return difference(
    ...
    grooveObj(height, 60, tolerance, radius).mirroredY().rotateZ(360/segments)
  )    
}
